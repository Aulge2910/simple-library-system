...
class User {	
	...
	public function listUsers(){		
		...
		$sqlQuery = "SELECT user_id, first_name, last_name, email, password, role
			FROM ".$this->userTable." ";			
			
		if(!empty($_POST["search"]["value"])){
			$sqlQuery .= ' WHERE (user_id LIKE "%'.$_POST["search"]["value"].'%" ';
		...
		while ($user = $result->fetch_assoc()) { 				
			...
			$rows[] = $user['email'];	
			$rows[] = $user['role'];			
			$rows[] = '<button type="button" name="update" id="'.$user["user_id"].'" class="btn btn-warning btn-xs update"><span class="glyphicon glyphicon-edit" title="Edit">Edit</span></button>';
			$rows[] = '<button type="button" name="delete" id="'.$user["user_id"].'" class="btn btn-danger btn-xs delete" ><span class="glyphicon glyphicon-remove" title="Delete">Delete</span></button>';
			$records[] = $rows;
			$count++;
		}
