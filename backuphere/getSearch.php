					...
					<div class="search-item col-lg-4 col-md-6 col-sm-6">
                        ...
                        <div class="search-item-detail">
                            ...
                            <span><?php echo $result[$k]["book_category"]; ?></span>
							 <span class="float-end"><?php echo $result[$k]["book_publisher"]; ?></span>
                            <p>Lorem ipsum, delectus quasi hic, error veniam aperiam et eius enim eveniet ipsa.</p>
                        </div>
                    </div>
...
<form name="frmSearch" method="post" action="search.php">
	<div>
		<p>
			<input type="hidden" id="rowcount" name="rowcount" value="<?php echo $_GET["rowcount"]; ?>" /><input type="text" placeholder="Name" name="name" id="name" class="search-input" value="<?php if (!empty($_POST["name"])) {
																																																		echo $_POST["name"];
																																																	} ?>" /><input type="text" placeholder="Code" name="code" id="code" class="search-input" value="<?php if (!empty($_POST["code"])) {
																																																																																							echo $_POST["code"];
																																																																																						} ?>" /><input type="button" name="go" class="btnSearch" value="Search" onclick="getresult('<?php echo $paginationlink . $page; ?>')"> <input type="button" class="btnReset" value="Reset" onclick="window.location='index.php'"> <span class="display-none"><img alt='Loader' src='spinner.svg' id="loader"></span>
		</p>
	</div>
	...
	<table class="stripped">
		...
		<tbody>
			...
			if (!empty($result)) {
				foreach ($result as $k => $v) {
			?>
					<tr id="toy-<?php echo $result[$k]["book_id"]; ?>">
						<td><?php echo $result[$k]["book_name"]; ?></td>
						<td><?php echo $result[$k]["book_name"]; ?></td>
						<td><?php echo $result[$k]["book_name"]; ?></td>
						<td><?php echo $result[$k]["book_name"]; ?></td>
						<td><?php echo $result[$k]["book_name"]; ?></td>
						<td><a class="mr-20 cursor-pointer" onClick="showEdit(this,<?php echo $result[$k]["book_id"]; ?>)">Edit</a>
							<a class="cursor-pointer" onClick="del(this,<?php echo $result[$k]["book_id"]; ?>)">Delete</a> <span class="display-none" id="action-loader"> <img alt='Loader' src='spinner.svg' class='align-middle'>
							</span>
						</td>
					</tr>
				<?php
